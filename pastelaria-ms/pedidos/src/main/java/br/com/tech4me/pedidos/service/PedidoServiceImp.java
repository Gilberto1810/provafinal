...
import br.com.tech4me.pedidos.httpClient.PastelariaClient;
...
public class PedidoServiceImp implements PedidoService{
    ...
    @Autowired
    private PastelariaClient pastelClient;
    ...
    public Optional<PedidoDto> obterPedidoPorId(String id) {
        ...
        if(pedido.isPresent()){
            PedidoDto pedidoComPastel = new ModelMapper().map(pedido, PedidoDto.class);
            pedidoComPastel.setDadosPastel(pastelClient.obterPastel(pedidoComPastel.getIdPastel()));
            return Optional.of(pedidoComPastel);
        }else{
